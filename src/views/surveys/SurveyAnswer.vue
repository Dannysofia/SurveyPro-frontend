  ...
  <section>
    ...
    <div v-else class="card">
      <div
        ...
        :style="{ background: survey.color || 'var(--primary)' }"
      ></div>
      <div class="pad-12">
        <div class="row-wrap" style="gap: 10px; align-items: center">
          <img
            v-if="survey.logoDataUrl"
            :src="survey.logoDataUrl"
            alt="logo"
            class="preview-logo"
          />
          <h2 class="title">{{ survey.title }}</h2>
          <span class="step" :class="{ active: survey.active }">{{
        ...
        <form v-else class="stack mt-8" @submit.prevent="onSubmit">
          ...
          >
            <label class="label"
              >{{ idx + 1 }}. {{ q.text }}
              <span v-if="q.required" class="error">*</span></label
            >
            ...
            <div v-else-if="q.type === 'multiple'" class="stack-sm">
              ...
              >
                <input
                  type="checkbox"
                  :value="o.id"
                  @change="onMultiChange(q.id, $event)"
                  :checked="isChecked(q.id, o.id)"
                />
                {{ o.text }}
              </label>
            </div>
...
const { getById, getByIdAsync, submitResponse } = useSurveys();

const surveyId = computed(() => String(route.params.id || ""));
const survey = computed(() => getById(surveyId.value));
const model = reactive({});

// Carga el detalle de la encuesta desde backend al montar
onMounted(async () => {
