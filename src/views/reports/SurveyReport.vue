  ...
  <section>
    <BreadCrumbsNav
      :items="[
        ...
        { label: 'Estadísticas', clickable: false },
      ]"
      @navigate="goTo"
    />
    ...
    <div class="list-header">
      ...
      <div class="row-wrap">
        <div class="export">
          <button class="btn btn-primary" @click="toggleExport">
            Exportar
          </button>
        ...
        <div class="card-body">
          ...
          <div v-if="stats[q.id]?.type === 'open'">
            <table class="table">
              ...
              <tbody>
                <tr
                  v-for="row in stats[q.id].rows"
                  :key="row.responseId + '_' + row.submittedAt"
                >
                  <td>{{ row.value }}</td>
                  <td>{{ formatDate(row.submittedAt) }}</td>
                </tr>
                <tr v-if="stats[q.id].rows.length === 0">
                  <td colspan="2" class="muted">Sin respuestas</td>
                </tr>
              </tbody>
            </table>
          </div>
          ...
          <div v-else-if="stats[q.id]?.type === 'single'">
            <div
              class="grid"
              style="grid-template-columns: 380px 1fr; gap: 16px"
            >
              <div style="min-height: 260px">
                <PieChart
                  :labels="stats[q.id].chart.labels"
                  :values="stats[q.id].chart.values"
                />
              </div>
              <div>
                <table class="table">
                  ...
                  <tbody>
                    <tr
                      v-for="row in stats[q.id].table"
                      :key="q.id + '_' + row.optionId"
                    >
                      <td>{{ row.value }}</td>
                      <td>{{ row.percentage.toFixed(1) }}%</td>
                      <td>{{ row.count }}</td>
                    </tr>
                    <tr v-if="stats[q.id].table.every((r) => r.count === 0)">
                      <td colspan="3" class="muted">Sin respuestas</td>
                    </tr>
                  </tbody>
                </table>
              </div>
...
async function ensureLoaded() {
  ...
  survey.value = await getByIdAsync(surveyId.value).catch(() =>
    getById(surveyId.value)
  );
